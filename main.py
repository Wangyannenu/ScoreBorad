    ...
    if op == 'load':
        ...
        if component_status[1][1] == 'no':
            ...
            if register_status[on2]:
                component_status[2][7] = register_status[on1]
...
def read_operands(run,clock,instructions,instructions_status,component_status,register_status):
    ...
    if op == 'load':
        if component_status[0][9] == 'yes':
            instructions_status[run][1] = clock
            # component_status[0][6] = ''
            # component_status[0][7] = ''
            # component_status[0][9] = 'yes'
    # elif register_status[on2] == '' and on2 in register_status and register_status[on1] == '' and on1 in register_status:#表示可以读取操作数
    elif op == 'mult':
...
def write_back (run, clock, instructions, instructions_status, component_status, register_status) :
    ...
    #     if instructions_status[i][1] > instructions_status[run][0] and instructions_status[i][1] <= clock:
    #         if instructions[i][2] == des or instructions[i][3] == des:
    #             return  # 如果有冲突，则跳过这次写回，等待下一次机会

    if pause == 0:
        instructions_status[run][3] = clock
        for i in range(len(component_status)):
            if op == component_status[i][2]:
