...
def socreboard(instructions):
    ...
    if instructions_count<len(instructions) and sign[instructions_count]==0:
        ...
        if op=='load':#已经确定load指令可以流出，在issue函数之中无需判断
            if component_status[0][0]==0 and register_status[des]=='':
                sign[instructions_count]=1
                instructions_count+=1
        elif op=='mult':
            if ( component_status[1][0]==0 or component_status[2][0]==0 )and register_status[des]=='':
                sign[instructions_count]=1
                instructions_count += 1
        elif op=='add'or op=='sub':
            if component_status[3][0]==0 and register_status[des]=='':
                sign[instructions_count]=1
                instructions_count += 1
        elif op=='divd':
            if component_status[4][0]==0 and register_status[des]=='':
                sign[instructions_count]=1
                instructions_count += 1
        run=0
        for _ in run:
            if sign[run]==1:
                if instructions_status[run][0]==0:
                    issue(run,clock,instructions,instructions_status,component_status,register_status)
                elif instructions_status[run][1]==0:
                    read_operands(run,clock,instructions,instructions_status,component_status,register_status)
                elif instructions_status[run][2]==0:
                    execute(run,clock,instructions,instructions_status,component_status,register_status)
                elif instructions_status[run][3]==0:
                    write_back(run,clock,instructions,instructions_status,component_status,register_status)
    ...
    if op=='load':
        ...
        if component_status[1][0]=='no':
            ...
            if register_status[on2]:#判断第二个操作数是否就绪
                ...
                component_status[1][6]=='no'
            elif register_status[on1]=='':
                component_status[1][6]=='yes'
